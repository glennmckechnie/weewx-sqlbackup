...
class SQLBackup(SearchList):
    ...
    #             log_success=True):
        ...
        weewx.conf
        """
        #self.cache_root = '/tmp/root/backups'

        #self.cache_root = self.generator.skin_dict['SQLBackup']['backup_root']

        #try:
        #    self.staleness_time = float(self.generator.skin_dict['SQLBackup']['staleness_time'])
        #except KeyError:
        #    self.staleness_time = 15 * 60 #15 minutes

        #try:
        #    self.refresh_time = float(self.generator.skin_dict['SQLBackup']['refresh_time'])
        #except KeyError:
        ...
        #try:
        #    xml_files = self.generator.skin_dict['SQLBackup']['xml_files']
        #except:
        #    xml_files = None
...
#        os.system("/usr/local/bin/mysql-backup-weewx.sh")
#        os.system("logger SQLBackup_init")

    #def run(self):
#        os.system("logger START_of_SQLBackup_init")
...
########### New Code !!! ########
#        os.system("bash /usr/local/bin/mysql-backup-weewx.sh")
#        os.system("/usr/local/bin/mysql-backup-weewx.sh")
#        os.system("logger WTF3")
#        os.system("logger SQLBackup_run_self")
        #feelslikeLocalList = weeutil.weeutil.option_as_list(self.generator.skin_dict['AusSearch']['feelslike']['feelslikeLocal'])

        my_user = "root"
        my_host = "localhost"
        my_pass = "h0whigh"
        my_database = "weatherpi"
        my_table = "archive"
        bup_dir = "/opt/backups/mysql-backups"
        dated_dir = 'yes'
#        kludge = ">"
        bup_file = ""
        _24hours = 86400 
        _48hours = 172800
        _7days = 604800
        # change the following to match the range selected
        t_period = "24hours"             # firstly, for the filename
        ...
        past_time = time.time() - float(_24hours)  # then for the dump process
#        past_time = str(1497166116)
#        past_time = "1497259000"
#        gt_past_time = " > %s" % past_time
        # If true, create the remote directory with a date structure
        # eg: <path to backup directory>/2017/02/12/var/lib/weewx...
        if self.dated_dir:
            date_dir_str = time.strftime("/%Y%m%d")
            ...
            date_dir_str = ''
        dump_bup_dir = bup_dir + "%s" % (date_dir_str)

#        dump_bup_dir = bup_dir + "%s/" + "%s.host-%s.%s-skiplock.gz"  % (date_dir_str, my_database, this_host,file_stamp)
# https://stackoverflow.com/questions/273192/how-can-i-create-a-directory-if-it-does-not-exist
        if not os.path.exists(dump_bup_dir):
            os.makedirs(dump_bup_dir)
        if weewx.debug >= 2:
            syslog.syslog(syslog.LOG_INFO, "sqlbackup: directory used for mysqldump - %s" % dump_bup_dir)

#        FILELOCK="$MBD/$db.host-$HOST.$NOW-skiplock.gz" 
        bup_file = dump_bup_dir + "/%s-host.%s-%s-%s.gz"  % (my_database, this_host, file_stamp, t_period)
        #cmd = ["/usr/bin/mysqldump", "-u" + my_user, "-p" + my_pass, "-h" + my_host, "-q",  my_database, my_table, '--where="dateTime>' + str( past_time) + '"', "--skip-opt", "--single-transaction"]
        #cmd = ["/usr/bin/mysqldump", "-u" + my_user, "-p" + my_pass, "-h" + my_host, "-q",  my_database, my_table, '-w"dateTime' + gt_past_time + '"', "--single-transaction", "--skip-opt"]
        # Can't grok the ">" properly? The WHERE string fails!
        #cmd = ["/usr/bin/mysqldump", "-u" + my_user, "-p" + my_pass, "-h" + my_host, "-q",  my_database, my_table, '-w"dateTime>' + past_time + '"', "--single-transaction", "--skip-opt"]
        ## this works with...
        ## p1 = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True) 
        #cmd = "/usr/bin/mysqldump -u%s -p%s -h%s -q  %s %s -w\"dateTime%s\" --single-transaction --skip-opt" %(my_user, my_pass, my_host, my_database, my_table, gt_past_time)
        cmd = "/usr/bin/mysqldump -u%s -p%s -h%s -q  %s %s -w\"dateTime>%s\" --single-transaction --skip-opt" %(my_user, my_pass, my_host, my_database, my_table, past_time)
#        print cmd

        #     perform the mysqldump ...
#        cmd = "/usr/bin/mysqldump -u" + my_user + " -h" + my_host + " -p" + my_pass + " --skip-opt --single-transaction -q " + my_database + " | /bin/gzip -9 > /tmp/teststringdump.gz"
#        cmd = ['/usr/bin/mysqldump -uroot -hlocalhost -ph0whigh --skip-opt --single-transaction -q weatherpi | /bin/gzip -9 > /tmp/teststringdump.gz']
#        os.system("/usr/bin/mysqldump -u%s -h%s -p%s --skip-opt --single-transaction -q %s | /bin/gzip -9 > %s" %(my_user,my_host, my_pass, my_database, bup_file))
#        cmd = ['/usr/bin/mysqldump']
#        cmd.extend(["-u%s" % my_user])
#        cmd.extend(["-p%s" % my_pass])
#        cmd.extend(["-h%s" % my_host])
#        cmd.extend(["--skip-opt"])
#        cmd.extend(["--single-transaction"])
#        cmd.extend(["-q"])
#        cmd.extend(["%s" % my_database])
#        cmd.extend(["|"])
#        cmd.extend(["/bin/gzip"])
#        cmd.extend(["-9"])
#        cmd.extend([">"])
#        cmd.extend(["%s" % bup_file])
#        cmd = "/usr/bin/mysqldump -u%s -h%s -p%s --skip-opt --single-transaction -q %s" % (my_user, my_host, my_pass, my_database)
#        cmd = "/usr/bin/mysqldump -u%s -h%s -p%s --skip-opt --single-transaction -q %s" % (my_user, my_host, my_pass, my_database)
#        gzip_cmd = " /bin/gzip -9 > %s" % (bup_file)
#        cmd = ["ls"]

        if weewx.debug >= 2:
            vetted_cmd = "/usr/bin/mysqldump -u%s -pXXXX -h%s -q  %s %s -w\"dateTime>%s\" --single-transaction --skip-opt" %(my_user, my_host, my_database, my_table, past_time)
            syslog.syslog(syslog.LOG_DEBUG, "sqlbackup: mysqldump cmd is ... %s" % (vetted_cmd))
...
#        dumpcmd = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
#        os.system("/usr/local/bin/mysql-backup-weewx.sh")
        ...
        t2= time.time() 
        syslog.syslog(syslog.LOG_INFO, "sqlbackup: Created %s backup in %s seconds" % (bup_file, t2-t1))

# date -d "11-june-2017 21:00:00" +'%s'
# 1497178800
#
# /usr/bin/mysqldump -uroot -ph0whigh -hlocalhost -q weatherpi  archive --where="dateTime>1497178800"
# archive --where="dateTime>1497103200"
...
#    rsync_upload.run()
    bup_dir = "/opt/backups/mysql-backups-test/"
    my_database = "weatherpi"

    sqlbackup = SQLBackup(bup_dir)
    sqlbackup()
